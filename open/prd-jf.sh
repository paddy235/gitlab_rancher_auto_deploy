...
services:
  $CI_PROJECT_NAME:
    ...
    labels:
      ...
      io.rancher.scheduler.affinity:container_label_ne: io.rancher.stack_service.name=open/$CI_PROJECT_NAME" > docker.yml
echo "version: '2'
services:
  $CI_PROJECT_NAME:
    scale: 1
...
/home/gitlab-runner/rancher-compose/rancher-compose --url $RANCHER_URL_PRD_JF --access-key $RANCHER_ACCESS_KEY_PRD_JF --secret-key $RANCHER_SECRET_KEY_PRD_JF -f docker.yml -r rancher.yml -p open up --force-upgrade --batch-size 1 --interval "30000" -d --confirm-upgrade
